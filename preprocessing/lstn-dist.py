...
class Video(object):
    ...
    def ApplyModel(self, ImagePath):
        ...
        if (results.face_landmarks):
            ...
            VERTICAL_PAD = 0.05
            distances = []
            for i in range(len(lips_x) - 1):
                p = [lips_y[i], lips_x[i]]
                q = [lips_y[i + 1], lips_x[i + 1]]
                dis = math.dist(p, q)
        ...
        print(distances)
        return lips_x, lips_y, distances
    ...
    def get_frames_mouth(self, frames):
        ...
        for i in range(len(frames)):
            ...
            lipsx, lipsy, distances = self.ApplyModel(frames[i])
            array = np.concatenate((lipsx, lipsy))
            array2 = np.concatenate((array, distances))
            print("array: ", array2.shape)
            fname.append(array2)
        return fname
...
def Save_files_as_zarr(data,count,directory):
  home=r'D:\Downloads\dataset\zarr'
