...
class DataAugmenter():
    ...
    def DataAugment(self):
        ...
        return np.array(verticalFrames),np.array(transform1Frames),np.array(transform4Frames)
...
class datasetMaker:
    ...
    def enumerate_videos(self, path,c1,c2):
        video_list = []
        print(path)
        # print("dishfisdfh")
        count1 = c1
    ...
    def get_batch(self, video_list, align_hash):
        ...
        for path in X_data_path:
            ...
            #verticalTransform,trans1,trans2=augmenter.DataAugment()
            # print("video data: ",video.data.shape)
            # print("Vertical transform: ",verticalTransform.shape)
            # print("transform 1: ", trans1.shape)
...
class Video(object):
    ...
    def getLipLandmarks(self, results):
        ...
        for lip_landmarks in results.landmark:
            ...
            if (index == 61 or index == 76 or index == 62 or index == 146 or index == 77 or index == 96
                ...
                lips_z.append(lip_landmarks.z)

            index += 1  # (Update)To keep track
        return lips_x, lips_y, lips_z
    ...
    def ApplyModel(self, ImagePath):
        ...
        if (results.face_landmarks):
            lips_x, lips_y, lips_z = self.getLipLandmarks(results.face_landmarks)
            self.prev_lipsx = lips_x
            self.prev_lipsy = lips_y
            self.prev_lipsz = lips_z
            print("lips_x",lips_x)
            print("lips_y", lips_y)
            print("lips_z", lips_z)
            ...
            #     p = [lips_y[features[i][0]], lips_x[features[i][0]],lips_z[features[i][0]]]
            #     q = [lips_y[features[i][1]], lips_x[features[i][1]],lips_z[features[i][1]]]
            #     dis = math.dist(p, q)
            #     print(type(lips_y[features[i][0]]))
            #     cv2.line(frame, (int(lips_x[features[i][0]]),int(lips_y[features[i][0]])), (int(lips_x[features[i][1]]),int(lips_y[features[i][1]])), (0, 255, 0), thickness=1, lineType=8)
            ...
            print("mouth_left: ", mouth_left)
            print("mouth_right: ", mouth_right)
            print("mouth_top: ", mouth_top)
            print("mouth_bottom: ", mouth_bottom)
            for i in range(len(lips_x)):
                lips_x[i] -= mouth_left
                lips_y[i] -= mouth_top
            print(lips_x)
            print(lips_y)
            for i in range(len(lips_x)):
                lips_x[i] = lips_x[i] / (mouth_right - mouth_left)
                lips_y[i] = lips_y[i] / (mouth_bottom - mouth_top)
            # # Unormalizing lip landmarks
            # for i in range(len(lips_x)):
            #     lips_x[i] = int(lips_x[i] * frame.shape[1])
            #     lips_y[i] = int(lips_y[i] * frame.shape[0])
            ...
            for i in range(len(lips_x)):
                ...
                while j<len(lips_x):
                    if(j!=i):
                        p = [lips_x[i],lips_y[i],lips_z[i]]
                        q = [lips_x[j],lips_y[j],lips_z[j]]
                        dis = math.dist(p, q)
                        distances.append(dis)
                    j+=1

            self.prev_distance = distances

        else:
            lips_x = self.prev_lipsx
            lips_y = self.prev_lipsy
            lips_z = self.prev_lipsz
            distances = self.prev_distance
        print("distances: ", len(distances))
        print(distances)
        return lips_x, lips_y,lips_z, distances
    ...
    def get_frames_mouth(self, frames):
        ...
        for i in range(len(frames)):
            ...
            lipsx, lipsy,lipsz, distances = self.ApplyModel(frames[i])
            array = np.concatenate((lipsx, lipsy))
            array2 = np.concatenate((array, lipsz))
            array3 = np.concatenate((array2, distances))
            print("array: ", array3.shape)
            fname.append(array3)
        return fname
    ...
    def set_data(self, frames):
        ...
        data_frames = np.array(frames)  # T x W x H x C
...
def Save_files_as_zarr(data,count,directory):
  home=r'D:\Downloads\dataset\zarr\lstm_norm'
  zarr.save(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_input.zarr',data[0]['the_input'])
  zarr.save(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_labels.zarr',data[0]['the_labels'])
  zarr.save(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_input_length.zarr',data[0]['input_length'])
  zarr.save(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_label_length.zarr',data[0]['label_length'])
  zarr.save(home+'\\'+directory+'_lstm_norm\\'+count+'\\ctc.zarr',data[1]['ctc'])

def Load_files_from_zarr(count,directory):
  home = r'D:\Downloads\dataset\zarr\lstm_norm'
  input = zarr.load(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_input.zarr')
  label = zarr.load(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_labels.zarr')
  input_length = zarr.load(home+'\\'+directory+'_lstm_norm\\'+count+'\\the_input_length.zarr')
  label_length = zarr.load(home+'\\'+directory+'_lstm_norm,\\'+count+'\\the_label_length.zarr')
  ctc = zarr.load(home+'\\'+directory+'_lstm_norm\\'+count+'\\ctc.zarr')
  inputs = {'the_input': input,
                  'the_labels': label,
                  'input_length': input_length,
                  'label_length': label_length,
...
if __name__ == '__main__':
    # CURRENT_PATH = r'D:\Downloads\dataset\0\0_test\*\*'
    # dataset = datasetMaker(CURRENT_PATH, CURRENT_PATH, 3, 100, 50, 75, 26)
    # video_list = dataset.enumerate_videos(CURRENT_PATH, 0, 5)
    # align_hash = dataset.enumerate_align_hash(video_list)
    # data = dataset.get_batch(video_list, align_hash)
    # directory = "01"
    # Save_files_as_zarr(data, str(1), directory)
    #

    CURRENT_PATH = r'D:\Downloads\dataset\4\4\*\*'
    ...
    directory = "4"
    count = 2
    video_list = dataset.enumerate_videos(CURRENT_PATH,count1,count2)
    align_hash = dataset.enumerate_align_hash(video_list)
    data = dataset.get_batch(video_list, align_hash)
